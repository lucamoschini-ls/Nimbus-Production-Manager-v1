...
export function DrawerStack({ drawers, onClose, onCloseUltimo, onOpenDrawer }: Props) {
  // Track recently closed for "reopen" button
  const [recentlyClosed, setRecentlyClosed] = useState<DrawerEntry[]>([]);

  // Esc closes last drawer
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape" && drawers.length > 0) {
        // Save before closing
        setRecentlyClosed(prev => [...prev.slice(-2), drawers[drawers.length - 1]]);
        onCloseUltimo();
      }
    };
  ...
  return (
    <div className="flex flex-shrink-0 border-l border-[#e5e5e7]">
      ...
      {drawers.map((d, idx) => (
        <div key={`${d.tipo}-${d.id}`} className="w-[360px] border-l border-[#e5e5e7] bg-white flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#e5e5e7] flex-shrink-0 bg-[#fafafa]">
            <div className="flex items-center gap-2">
              {idx === 0 && recentlyClosed.length > 0 && (
                <button onClick={handleReopen} className="text-[#86868b] hover:text-[#1d1d1f]" title="Riapri precedente">
                  <ArrowLeft size={14} />
                </button>
              )}
              <span className="text-[11px] text-[#86868b] font-medium uppercase">{d.tipo}</span>
            </div>
            <button onClick={() => handleClose(idx)} className="text-[#86868b] hover:text-[#1d1d1f] p-0.5 rounded hover:bg-[#f0f0f0]">
              <X size={14} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {d.tipo === "materiale" && <DrawerMateriale id={d.id} onOpenTask={(taskId) => onOpenDrawer("task", taskId)} />}
            {d.tipo === "task" && <DrawerTask id={d.id} onOpenMateriale={(matId) => onOpenDrawer("materiale", matId)} />}
