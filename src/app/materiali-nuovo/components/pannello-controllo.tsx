...
import type { Raggruppamento, FinestraTemporale, SuperficieState } from "../hooks/use-superficie-state";
...
export function PannelloControllo({ state, onRaggruppa, onToggleCat, onFinestra, onCerca, onPreset }: Props) {
  const presetMatch = (p: string) => {
    if (p === "acquisti") return state.raggruppa === "fornitore" && state.finestra === "settimana";
    if (p === "cantiere") return state.raggruppa === "zona" && state.finestra === "oggi";
    if (p === "catalogo") return state.raggruppa === "nessuno" && state.finestra === "stagione" && state.filtriCat.length === 0;
    return false;
  };
  ...
  return (
    <div className="w-[280px] flex-shrink-0 bg-[#fafafa] border-r border-[#e5e5e7] overflow-y-auto p-4 space-y-5">
      {/* Preset */}
      <div>
        <div className="text-[10px] text-[#86868b] font-semibold uppercase tracking-wide mb-2">Preset</div>
        <div className="flex gap-1.5">
          {(["acquisti", "cantiere", "catalogo"] as const).map(p => (
            <button key={p} onClick={() => onPreset(p)}
              ...
              {p === "cantiere" ? "Cantiere" : p === "acquisti" ? "Acquisti" : "Catalogo"}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Raggruppa per */}
      <div>
        <div className="text-[10px] text-[#86868b] font-semibold uppercase tracking-wide mb-2">Raggruppa per</div>
        <div className="space-y-1">
          {RAGGRUPPAMENTI.map(r => (
            <label key={r.value} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="raggruppa" checked={state.raggruppa === r.value}
      ...
      <div>
        <div className="text-[10px] text-[#86868b] font-semibold uppercase tracking-wide mb-2">Categoria</div>
      ...
      {/* Finestra temporale */}
      <div>
        <div className="text-[10px] text-[#86868b] font-semibold uppercase tracking-wide mb-2">Finestra</div>
        <div className="space-y-1">
          {FINESTRE.map(f => (
            <label key={f.value} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="finestra" checked={state.finestra === f.value}
      ...
      {/* Ricerca */}
      <div>
        <div className="text-[10px] text-[#86868b] font-semibold uppercase tracking-wide mb-2">Cerca</div>
