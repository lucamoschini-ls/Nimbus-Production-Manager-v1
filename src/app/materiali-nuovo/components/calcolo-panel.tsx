...
import type { DriverRow, CoefficienteRow } from "../materiali-superficie";
    ...
    <div className="flex items-center gap-2 py-1.5">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1">
          ...
          {item.tooltip && <InfoTooltip variant="info" text={item.tooltip} />}
      ...
      <input
        ...
        onBlur={handleBlur}
        placeholder={hasDefault ? String(item.valore_default) : undefined}
...
export function CalcoloPanel({
  ...
  const handleSaveDriver = async (driverId: string, valore: number) => {
    onUpdateDriver(driverId, valore);
    try {
      ...
      <div className="border-t border-[#f0f0f0] pt-2">
        ...
        {driverGroups.map(([group, items]) => (
          <GroupSection key={group} title={group}>
            {items.map((d) => (
              <CalcoloInput key={d.id} item={d} onSave={handleSaveDriver} />
            ))}
          </GroupSection>
        ))}
      </div>
