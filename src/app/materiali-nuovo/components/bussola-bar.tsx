...
export function BussolaBar({ state, onReset }: Props) {
  const segments: string[] = ["Materiali"];
  if (state.raggruppa !== "nessuno") segments.push(RAGGRUPPA_LABELS[state.raggruppa]);
  if (state.filtriCat.length > 0) segments.push(`Filtro: ${state.filtriCat.join(", ")}`);
  ...
  return (
    <div className="flex-shrink-0 h-[50px] bg-white border-b border-[#e5e5e7] flex items-center px-4 gap-4 z-30">
      ...
      <div className="flex items-center gap-1.5 flex-1 min-w-0">
        {segments.map((s, i) => (
          <span key={i} className="flex items-center gap-1.5">
            ...
            <span className={`text-[13px] ${i === segments.length - 1 ? "text-[#1d1d1f] font-medium" : "text-[#86868b] hover:text-[#1d1d1f] cursor-pointer"}`}>
              {s}
            </span>
          </span>
        ))}
      ...
      <div className="flex items-center gap-3 text-[12px] text-[#86868b] flex-shrink-0">
        <span>192 voci</span>
        <span className="text-red-500 font-medium">12 in rosso</span>
        <span className="font-medium text-[#1d1d1f]">1.847 &euro; da comprare</span>
      </div>

      {/* Reset */}
      <button onClick={onReset} className="flex items-center gap-1 text-[11px] text-[#86868b] hover:text-[#1d1d1f] px-2 py-1 rounded-md hover:bg-[#f5f5f7] flex-shrink-0">
