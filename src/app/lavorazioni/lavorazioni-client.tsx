...
const TIPOLOGIA_COLORS: Record<string, string> = {
  carpenteria: "bg-orange-100 text-orange-700",
  verniciatura: "bg-purple-100 text-purple-700",
  elettrico: "bg-yellow-100 text-yellow-700",
  idraulico: "bg-cyan-100 text-cyan-700",
  trasporto: "bg-stone-100 text-stone-600",
  acquisto: "bg-pink-100 text-pink-700",
  montaggio: "bg-indigo-100 text-indigo-700",
  audio_luci: "bg-fuchsia-100 text-fuchsia-700",
  giardinaggio: "bg-lime-100 text-lime-700",
  pulizia_manutenzione: "bg-teal-100 text-teal-700",
  decisione: "bg-sky-100 text-sky-700",
  amministrativo: "bg-slate-100 text-slate-600",
...
export function LavorazioniClient({ zone, lavorazioni, tasks, fornitori, tipologie }: Props) {
  ...
  const selectedZona = selectedLavData
    ? zone.find((z) => z.id === selectedLavData.zona_id)
    : null;
  ...
  function getAttesaMotivo(t: TaskCompleta): string | null {
    ...
    if (t.stato_calcolato === "in_attesa_materiali") {
      return `Attesa: ${t.materiali_mancanti} materiali non in cantiere`;
    }
    if (t.stato_calcolato === "in_attesa_permesso") {
      return "Attesa: permesso non ottenuto";
    }
    return null;
  }
            ...
            return (
              <div key={z.id}>
                ...
                {isExpanded && (
                  <div className="ml-4">
                    {zoneLav.map((lav) => {
                      const lavTasks = tasks.filter(
                        (t) => t.lavorazione_id === lav.id
                      );
                    ...
                    {addingLavTo === z.id ? (
                      <form
                        ...
                        onSubmit={async (e) => {
                          e.preventDefault();
                          if (newLavName.trim()) {
                            await createLavorazione({ zona_id: z.id, nome: newLavName.trim() });
                            setNewLavName("");
                            setAddingLavTo(null);
                          }
                        }}
                      >
                        <input
                          autoFocus
                          value={newLavName}
                          onChange={(e) => setNewLavName(e.target.value)}
                          placeholder="Nome lavorazione"
                          className="flex-1 text-xs border border-[#e5e5e7] rounded px-2 py-1 outline-none focus:ring-1 focus:ring-ring"
                          onBlur={() => { if (!newLavName.trim()) setAddingLavTo(null); }}
                        />
                      </form>
                    ) : (
                      <button
                        onClick={() => setAddingLavTo(z.id)}
                        className="w-full flex items-center gap-1 px-4 py-1.5 text-[10px] text-[#86868b] hover:text-[#1d1d1f]"
                      >
        ...
        <div className="flex-1 overflow-y-auto p-6" ref={taskAreaRef}>
          {selectedLavData ? (
            <>
                  ...
                  {selectedZona && (
                    <>
                      <span
                        ...
                        style={{ backgroundColor: selectedZona.colore }}
                      />
                      <span>{selectedZona.nome}</span>
                      <span>/</span>
                    </>
                  )}
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <div className="flex-1 max-w-xs h-1.5 bg-[#e5e5e7] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.round(
                            (selectedLavTasks.filter(
                              (t) => t.stato_calcolato === "completata"
                            ).length /
                              selectedLavTasks.length) *
                              100
                          )}%`,
                        }}
                      />
                    ...
                    <span className="text-xs text-[#86868b]">
                      {selectedLavTasks.filter((t) => t.stato_calcolato === "completata").length}/
                      {selectedLavTasks.length} completate
                    </span>
                  </div>
                )}
              </div>
              ...
              {addingTaskTo === selectedLav ? (
                <form
                  ...
                  onSubmit={async (e) => {
                    e.preventDefault();
                    if (newTaskTitle.trim() && selectedLav) {
                      await createTask({
                        lavorazione_id: selectedLav,
                ...
                <button
                  onClick={() => setAddingTaskTo(selectedLav)}
                  className="mt-3 flex items-center gap-1.5 text-sm text-[#86868b] hover:text-[#1d1d1f] transition-colors"
                >
            ...
            <div className="flex items-center justify-center h-64 text-[#86868b] text-sm">
              Seleziona una lavorazione dalla sidebar
            </div>
          )}
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {zone.map((z) => {
                const zoneTasks = tasks.filter((t) =>
                  lavorazioni
                    .filter((l) => l.zona_id === z.id)
                    .some((l) => l.id === t.lavorazione_id)
                );
                const completed = zoneTasks.filter((t) => t.stato_calcolato === "completata").length;
                const pct = zoneTasks.length > 0 ? Math.round((completed / zoneTasks.length) * 100) : 0;
                return (
                ...
                .map((lav) => {
                  ...
                  return (
                    <button
                      key={lav.id}
                      onClick={() => {
                        setSelectedLav(lav.id);
                        setMobileView("tasks");
                      }}
                      className="w-full bg-white rounded-[12px] border border-[#e5e5e7] p-4 text-left flex items-center justify-between"
                    >
                      <span className="text-sm font-medium text-[#1d1d1f]">{lav.nome}</span>
                      <span className="text-xs text-[#86868b]">{lavTasks.length} task</span>
                    </button>
                  );
                })}
            </div>
